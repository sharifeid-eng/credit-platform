...
- **Tamara** — Buy Now Pay Later (BNPL + BNPL+), KSA & UAE. Saudi Arabia's first fintech unicorn ($1B valuation, 20M+ users, 87K+ merchants). **Hybrid ingestion** — (1) one-off data room ETL via `scripts/prepare_tamara_data.py` (session 19, ~100 source files incl vintage cohort matrices, Deloitte FDD, HSBC investor reports) for the comprehensive baseline snapshot, PLUS (2) recurring structured quarterly pack via `scripts/ingest_tamara_investor_pack.py` (session 37) that parses Tamara's 10-sheet Excel template into `data/Tamara/investor_packs/YYYY-MM-DD_investor_pack.json`. The recurring channel auto-fires the investment thesis drift check on ingest. Two products: KSA (SAR, 19 tabs incl "Quarterly Financials") and UAE (AED, 15 tabs incl "Quarterly Financials"). Dashboard: `TamaraDashboard.jsx`. Parser: `core/analysis_tamara.py`. Thesis: 8 pillars in `data/Tamara/mind/thesis.json` (seeded via `scripts/seed_tamara_thesis.py`). Config: `analysis_type: "tamara_summary"`. Securitisation: KSA $2.375B (Goldman, Citi, Apollo), UAE $131M (Goldman). Live dataset: `data/Tamara/{KSA,UAE}/` + investor packs at `data/Tamara/investor_packs/`
...
│   ├── test_population_discipline_guard.py  # Session 34 — §17 covenant+limit dict shape contract (9 tests)
│   ├── test_population_discipline_meta_audit.py # Session 35 — §17 rate-field disclosure walker + dual-view propagation + taxonomy freshness (8 tests)
│   ├── test_population_audit_2026_04_22.py  # Session 34+35 — P0/P1/P2/UNCERTAIN-gap regression tests from the original audit (100+ tests)
├── scripts/
...
│   ├── prepare_tamara_data.py  # Data room ETL: reads ~100 source files → structured JSON snapshots for Tamara (ONE-OFF — see ingest_tamara_investor_pack.py for recurring quarterly channel)
│   ├── ingest_tamara_investor_pack.py  # Session 37 — recurring quarterly/monthly Tamara investor-pack ingest. Parses 10-sheet template → data/Tamara/investor_packs/YYYY-MM-DD_investor_pack.json. Auto-fires thesis drift check on each pack unless --no-update-thesis.
│   ├── seed_tamara_thesis.py   # Session 37 — seeds data/Tamara/mind/thesis.json with 8 pillars. Idempotent; preserves created_at + pillar IDs on re-seed.
│   ├── prepare_aajil_data.py   # Investor deck extraction → structured JSON snapshot for Aajil
│   ├── dataroom_ctl.py         # Unified dataroom operator CLI (audit/ingest/refresh/rebuild-index/wipe/classify). Session 37 — classify subcommand hardened: passes sheet_names for xlsx + --dry-run flag + improved --use-llm guidance.
...
- **Recurring Channel Detection (Mind hierarchy mapping)** — `core/mind/pattern_detector.py` auto-detects when a company's dataroom accumulates ≥3 files of the same `document_type`. The Mind-tier mapping is non-negotiable: **Company Mind is PRIMARY storage** (one entry per `company × document_type` pattern in `mind/recurring_channels.jsonl`, idempotent on `pattern_id`, status-transition rows preserve audit history). **Asset Class Mind candidates are SURFACED, never auto-written** — analyst promotes via the existing `core/mind/promotion.py` infrastructure (Operator Center → Mind tab → "Promote to Asset Class"). **Master Mind only gets ONE rolling stats file** at `data/_master_mind/recurring_channel_stats.json` (overwritten, not appended) — strategic visibility of "X automated, Y candidates, Z early" without per-pattern pollution. Auto-fired post-ingest by `dataroom_ctl ingest`/`refresh` via `auto_fire_after_ingest()` (best-effort, never breaks deploy on detector failure). Status taxonomy: AUTOMATED (≥3 files + hook + parser), PARTIAL (one of hook/parser), CANDIDATE (≥3 files, neither — most actionable), EARLY (1-2 files). Surfaced in Operator Center "Channels" tab with copy-pasteable Claude session prompts for CANDIDATE/PARTIAL clusters. See `tests/test_pattern_detector.py` for the full contract (34 tests).
...
- **`deploy.sh` uses `dataroom_ctl needs-ingest` to detect both registry corruption AND new source files** — session-37 lesson. The old inline `registry_count == chunk_count` alignment check was satisfied (e.g. 271==271) when sync-data.ps1 dropped fresh files on prod, so deploy silently skipped ingest and analysts saw stale dashboards. New `needs-ingest` subcommand (in `scripts/dataroom_ctl.py`) checks alignment AND compares source-file mtimes against `ingest_log.jsonl` mtime. Engine-written sidecars + dotfiles + `chunks/`/`analytics/` subdirs are excluded via the same `_is_supported()` filter the engine uses for ingest — single source of truth for the exclusion list. Exit codes inverted vs `audit` (0=ingest needed, 1=clean) to fit the `if needs-ingest; then ingest; fi` bash idiom.
- **Generic post-ingest hooks (`data/{co}/dataroom/.post-ingest.sh`)** — session-38 lesson. After `dataroom_ctl ingest` lands successfully for a company, deploy.sh checks for a per-company hook script and runs it via `bash "$hook"` (no executable bit required — Windows clones don't reliably preserve it). Hook failures are logged but never fail the deploy. Hook only fires after a successful ingest; skipped when needs-ingest reports clean OR when ingest itself failed. Hooks ship via git (`!data/*/dataroom/.post-ingest.sh` negation in `.gitignore` — they're deployment infrastructure, not analyst data, so the dual-rail collision pattern from session 26.1 doesn't apply). First consumer: Tamara — auto-parses the newest investor pack file in `data/Tamara/dataroom/Financials/54.2.2 Management Financials/` via `scripts/ingest_tamara_investor_pack.py --force`, which auto-fires the thesis drift check by default. Pattern is forward-compatible: when SILQ or another company eventually needs post-ingest processing, they get the same plumbing for free — drop a `.post-ingest.sh` in their dataroom dir.
...
-----
## What's Working
- ✅ **Recurring Channel Detection — auto-detects + classifies recurring data channels with full Mind-tier integration (2026-04-26, branch `claude/recurring-channel-detection-v2`):**
  - **Trigger:** spec-driven request to "stop needing the analyst to spot recurring channels — make the platform spot them itself, ask for help only when human judgment is required (which generalisations to elevate, when to invest in automation)." Tamara accumulating 5+ quarterly investor packs over time was the motivating example — that pattern was discovered manually in session 37.
  - **Architecture:** new `core/mind/pattern_detector.py` (~430 lines) is pure-function detection. `detect_recurring_patterns(company)` reads `data/{company}/dataroom/registry.json`, groups by `document_type` (skipping `other`/`unknown`), classifies each cluster by `(file_count, hook_exists, parser_exists)` into AUTOMATED / PARTIAL / CANDIDATE / EARLY. `detect_emergent_asset_class_patterns()` walks every company and surfaces (asset_class, document_type) combos shared by ≥2 companies — does NOT auto-write to Asset Class Mind, only surfaces candidates. `auto_fire_after_ingest()` is the wrapper that catches every exception so detection failures NEVER break ingest/refresh.
  - **Mind-tier integration:** `write_patterns_to_company_mind()` writes to `data/{company}/mind/recurring_channels.jsonl` via the new `CompanyMind.record(category, content, metadata)` generic method (mirrors MasterMind/AssetClassMind's existing `record()` API). Idempotent on `pattern_id`: only appends new entries when `automation_status` OR `file_count` differs from the latest existing entry — status transitions (e.g. CANDIDATE → AUTOMATED after analyst sets up the hook) preserve history. `recurring_channels` added to CompanyMind `_FILES` but deliberately omitted from `_TASK_RELEVANCE` so operational records don't pollute AI prompt context. `write_fund_wide_stats()` overwrites a single rolling JSON at `data/_master_mind/recurring_channel_stats.json` (no per-pattern entries in Master Mind — strategic visibility only).
  - **CLI surface:** new `dataroom_ctl detect-patterns [--company X] [--emergent]` subcommand. `cmd_ingest` and `cmd_refresh` now auto-fire `_auto_fire_pattern_detection(company)` after success — best-effort, JSON payload includes `pattern_detection: {patterns_detected, new_mind_entries, fund_stats_updated}`. Failures land as `pattern_detection.error` and never abort the deploy.
  - **Backend endpoints:** `GET /api/operator/recurring-channels` (per-company patterns + summary), `GET /api/operator/emergent-patterns` (cross-company candidates). Both compute on-demand from registry — independent of Mind state — so the UI always reflects current dataroom contents.
  - **Frontend:** new "Channels" tab in OperatorCenter.jsx (12th tab) with two sections: per-company detection cards (status pills, file count, cadence, hook/parser badges, "Copy setup prompt" button on CANDIDATE/PARTIAL) and emergent asset-class cards (purple accent, EMERGENT badge, hand-off note pointing at the Mind tab promotion flow). Summary strip with 5 stats: Automated / Partial / Candidates / Early / Emergent. Tab-button count badge sums CANDIDATE patterns + emergent candidates (most-actionable).
  - **Smoke test (real data):** `python scripts/dataroom_ctl.py detect-patterns --company Tamara --emergent` produced 17 patterns: 1 AUTOMATED (`quarterly_investor_pack`, 4 files, hook + parser detected via the session-37 post-ingest hook + `scripts/ingest_tamara_investor_pack.py` parser); 8 PARTIAL (vintage_cohort 47, investor_report 23, demographics 17, financial_statement 10, company_presentation 7, business_plan 3, kyc_compliance 3, legal_document 3 — all with hook present, no specific parser); 8 EARLY (1-2 files). 0 emergent (no other company in data/ has a registry yet — SILQ is the only other one, and it's `pos_lending` not `bnpl`). SILQ smoke separately confirmed CANDIDATE classification: 2 CANDIDATEs (investor_report 4, portfolio_tape 4 — no hook + no parser).
  - **Tests:** new `tests/test_pattern_detector.py` (34 tests in 9 classes) covers thresholds, classification (4 status labels), cadence buckets (monthly/quarterly/annual/irregular/unknown), parser-detection word-overlap heuristic (false-positive prevention), sort order (CANDIDATE → PARTIAL → EARLY → AUTOMATED), Mind integration (idempotency + status-transition history), emergent patterns (≥2 companies, asset_class grouping, no auto-write to Asset Class Mind), fund-wide stats (single rolling file), auto-fire safety (detector exception → returns dict with `error`, never raises). Full suite: **829 passed, 38 skipped, 0 warnings, 0 regressions** (was 795 + 34 new). Frontend `vite build` green (1110 modules).
  - **Acceptance criteria:** drop a 3rd file of any document_type into ANY company's dataroom → run sync-data.ps1 + ./deploy.sh → that pattern auto-appears in Operator Center → Channels → Per-Company Detections as CANDIDATE with copy-pasteable Claude session prompt to set up parser + hook. Cross-company emergence surfaces when 2+ companies in the same `asset_class` show 3+ same-type files.
  - **Constraints honored:** detection NEVER auto-writes to Asset Class Mind; analyst always promotes via `core/mind/promotion.py`. Master Mind only gets the rolling stats file, never per-pattern entries. CompanyMind extended additively (new `record()` method + new `recurring_channels` category) — existing record_* methods + categories untouched, all 27 existing mind tests still pass.
- ✅ **deploy.sh `needs-ingest` detection — session-37 footgun fix (2026-04-25, branch `claude/deploy-sh-needs-ingest-detection` — review-gated, NOT merged to main):**
  - **Trigger:** session-37 wrap-up flagged the silent failure where `sync-data.ps1` dropped fresh source files on prod, deploy.sh's old inline `registry_count == chunk_count` alignment check was satisfied (271==271), and ingest got skipped — leaving new files un-chunked while dashboard showed stale data. Patched in session 37 EoD step 11 with a markdown nudge; this branch is the architectural fix.
  - **Architecture:** new `needs-ingest` subcommand in `scripts/dataroom_ctl.py` exits 0 if ingest needed, 1 if clean (inverted vs `audit` to fit `if needs-ingest; then ingest; fi` bash idiom). Two trigger conditions: (a) registry corruption (no/empty registry, registry vs chunks mismatch); (b) any source file newer than `ingest_log.jsonl` mtime. Source-file walk imports `_is_supported()` from `core.dataroom.engine` — exclusion list lives in ONE place (no bash duplicate that would drift on every onboard).
  - **deploy.sh rewrite:** iterates `data/*/dataroom/` (any dataroom, not just those with registry.json), delegates per-company to `needs-ingest`, ingests on exit 0, prints "skipping" on exit 1. Bash syntax validated.
  - **Tests:** 12 new `TestNeedsIngest` cases (target was 8) covering aligned+no-new = clean, new-source-file = newer_files, registry/chunks mismatch, no ingest_log, no registry, empty dataroom, excluded sidecars + dotfiles + chunks/ files don't trigger, real .json source files DO trigger, top-level help lists subcommand, subcommand help documents exit-code semantics. Full suite **786 passing, 0 warnings, 0 regressions** (was 774 + 38 baseline).
  - **Smoke-tested against real Tamara dataroom** (137 source files, 129 aligned registry+chunks): baseline `clean` → `touch _smoke_test_fake.pdf` → `needs_ingest: true, reason: newer_files, newer_count: 1` → `rm` → back to `clean`. Tamara post-smoke audit confirmed `aligned: true, registry=129 chunks=129`.
  - **Commit:** `b5a753a` on `claude/deploy-sh-needs-ingest-detection`. PR-ready URL: https://github.com/sharifeid-eng/credit-platform/pull/new/claude/deploy-sh-needs-ingest-detection. Branch pushed but **not merged** per spawning prompt — analyst review-gated. Ship to prod requires merging to main + running `./deploy.sh` on Hetzner VPS.
...
- ✅ **Session 36 — 8-gap follow-up sweep after §17 audit waves (2026-04-24):**
  ...
  - **4 commits landed on `main`** (`f7ef580..074f430`):
    1. **`f7ef580`** — `feat(§17)`: per-row meta-audit walker + analytics_bridge disclosure transmission.
       - New `TestMetaAuditPerRowRateFieldDisclosure` class in `tests/test_population_discipline_meta_audit.py`. Session 35's scalar walker caught 45 top-level gaps but left per-row rates inside dict-nested lists (`vintages[]`, `by_product[]`, `transition_matrix[]`) unaudited. New walker recurses into list values and accepts 5 disclosure modes (dict-level inheritance, list-level `<list>_population`, row-level, per-field, documented exemption).
       - First run surfaced **19 additional gaps in 7 compute functions** — all fixed in-file: Klaim `compute_facility_pd` (transition_matrix + distribution + facility_pd declarations), Klaim `compute_cohort_loss_waterfall.vintages`, SILQ `compute_silq_concentration.utilization / product_mix / size_distribution`, SILQ `compute_silq_tenure.distribution`, SILQ `compute_silq_borrowing_base.by_product`, SILQ `compute_silq_cohort_loss_waterfall.vintages`, Aajil `compute_aajil_underwriting.vintages`, Aajil `compute_aajil_yield.by_deal_type / by_vintage`.
       - New `_get_disclosure(dct, field_name)` + `_attach_disclosure(metric, dct, field_name)` helpers in `core/memo/analytics_bridge.py`. Only `_build_covenants` was transmitting confidence/population to memo prompts; 5 non-covenant builders (portfolio_analytics, credit_quality, concentration, stress, covenants) now all flow §17 disclosure end-to-end. Family-level inheritance handles `par_*`, `hhi_*`, `collection_rate_*`.
       - Smoke-tested live: Klaim `PAR 30 [conf=A, pop=active_outstanding]`, `Recovery Rate [conf=A, pop=loss_subset]`, `Net Loss Rate [conf=A, pop=total_originated]` flowing through the bridge to memo prompts.
    2. **`7d68c9c`** — `docs(§17)`: lifetime-primary universal PAR rule + taxonomy revision + Ejari exemption.
       - Revised §17 "Dual-view pattern taxonomy" in `core/ANALYSIS_FRAMEWORK.md`. Session 35 codified Pattern 1 (active-primary) as asset-class-dependent. Session 36 override: **for PAR family specifically, lifetime is ALWAYS primary across all asset classes**. Rationale: cross-company IC audience consistency (analysts compare Klaim's lifetime PAR to SILQ's to Aajil's without mentally re-normalising); lifetime captures what active hides (active resets as bad deals close out); active remains available for covenant compliance unchanged.
       - `core/FRAMEWORK_INDEX.md` — added §17 cross-references (principle propagation + pattern taxonomy) to the section-index table + new Core Principle #8 "Credit Quality PAR: lifetime-primary universal", renumbered subsequent principles.
       - `core/analysis_ejari.py::parse_ejari_workbook` docstring — added explicit §17 exemption note. Ejari workbook is pre-computed by upstream; parser reshapes aggregated data. Upstream's denominator choice is documented in Notes sheet + `methodology_ejari.py`. Meta-audit walker correctly omits Ejari functions.
    3. **`4a43f02`** — `feat(ui)`: flip Credit Quality PAR to lifetime-primary + ConfidenceBadge on charts.
       - `TapeAnalytics.jsx` SILQ Credit Quality section flipped — headline now `summary.lifetime_par30 ?? summary.par30`, subtitle `{at-risk amount} · Active: X.XX%`, section sublabel "vs Total Disbursed (lifetime)". Color thresholds retuned for lifetime scale. Klaim section already lifetime-primary; enhanced subtitle to match SILQ pattern + added confidenceNote. `AajilDashboard.jsx` Delinquency tab: PAR 1+/2+/3+ Inst cards flipped to `par_N_inst_lifetime` primary with `par_N_inst` shown as "Active: X.X%" subtitle + confidence/population declarations. Ejari (single-PAR upstream — §17-exempt) and Tamara (DPD distribution, no dual cards) correctly skipped.
       - ConfidenceBadge wired via ChartPanel's `action` slot into 8 chart panels: `CohortTable`, `ConcentrationChart` (Group + Provider), `CohortLossWaterfallChart`, `ReturnsAnalysisChart` (IRR by Vintage + Monthly Returns), `UnderwritingDriftChart`, `silq/DelinquencyChart` (DPD + Monthly Trend). §17 discipline now visible on charts, not just overview KPIs.
       - Vite build green (1110 modules transformed). API data-flow confirmed: SILQ lifetime_par30=1.45% vs active 7.81%, Klaim 2.37% vs 36.59%, Aajil par_1_inst_lifetime=0.07 vs active 0.33.
    4. **`074f430`** — `docs(session 36)`: `tasks/next-session-kickoff.md` refreshed.
       - Archived stale session-27 external.web_search smoke prompt as a `<details>` block. Main body replaced with session-36 carry-over: visual verification handoff checklist (per-company PAR flip verification steps + chart badge hover checks), status summary of all 8 gaps, pattern guidance for future audits (two §17 walkers + 4 disclosure-fix patterns).
  - **Design tension resolved (Gap 8a):** session 35 had codified Pattern 1 (active-primary for lending) as the PAR convention for SILQ. Session 36 user directive to flip to lifetime-primary conflicted. Option (a) chosen — revise §17 taxonomy to make lifetime-primary universal for PAR family rather than keep asset-class-dependent Pattern 1. Preserves consistency between framework doctrine and UI without UI-override caveats.
  - **Visual verification** (Gap 4): compile-verified + API data-flow verified; full browser click-through handed off to user via session-36 kickoff checklist per CLAUDE.md guidance ("Type checking and test suites verify code correctness, not feature correctness — if you can't test the UI, say so explicitly").
  - **Tests: 743 passing** (baseline maintained), 0 warnings, 0 regressions.
  - **3 new lessons** in `tasks/lessons.md`: (1) walkers that only audit top-level fields miss per-row disclosures; (2) multiple concurrent Claude Code sessions share working tree state during branch switches (discipline: never use `git add -A` when other sessions run on same repo); (3) doctrine revisions must propagate to UI + compute declarations in the same landing.
- ✅ **Session 37 — Tamara quarterly investor pack pipeline (A→D + E+F + CLI hardening, 2026-04-24):**
  - **Trigger:** User delivered two new Tamara documents — `2026-04-16_credit_risk_portfolio_update.pdf` (narrative credit update) and `2. 1Q2026 Tamara Cons. Investor Pack.xlsx` (structured 10-sheet template, 27 months monthly through Mar-26). Analysis confirmed the Excel uses the SAME template as 4 prior files in the dataroom (Nov'25, Dec'25 ×2, Jan'26) — recurring quarterly/monthly reporting, not one-off. Opportunity to build a **repeatable quarterly structured ingest pipeline** that replaces the one-off `prepare_tamara_data.py` ETL.
  - **Jobs A-D** (commit `9d14144`): dataroom ingest + classifier enum + parser script + enrichment. `dataroom_ctl ingest --company Tamara`: 137 seen, 129 added, 135 stale orphans pruned, `aligned=registry=129 chunks=129`. New `DocumentType.QUARTERLY_INVESTOR_PACK` with filename rule (ordered before INVESTOR_REPORT to win precedence) + sheet-name fallback matching the 10-sheet template. Frontend `DocumentLibrary` gold-accented pill signals first-class recurring data input. New `scripts/ingest_tamara_investor_pack.py` (383 lines): template validation, 3-tier pack-date resolution (CLI > heuristic from data range > file mtime), deterministic sorted-key JSON output. Real file → 424KB JSON at `data/Tamara/investor_packs/2026-04-15_investor_pack.json` with 74/49/49 KPI line items + 118/91/89 FS items + 7 budget variance rows. New `_enrich_quarterly_pack()` in `core/analysis_tamara.py` merges pack under `quarterly_pack` key with headline MoM deltas (13 FS + 12 KPI per country) + 6-metric YTD budget variance summary. Graceful no-op when pack folder missing. Real-data smoke: Mar-26 Statutory Net Profit -42% MoM; YTD EBTDA +124% vs budget.
  - **Jobs E+F** (commit `e333595`): frontend tab + thesis pillars + auto-drift. `TamaraDashboard.jsx` gains 4 new sub-components (`ThesisSummaryCard`, `QuarterlyHeadlineCard`, `BudgetVarianceRow`, `QuarterlyFinancialsPanel`) rendering a 3-country toggle (Consolidated/KSA/UAE) + 10 FS headline cards with color-coded MoM deltas + 8 KPI cards + 6-metric budget variance table + thesis pillar status grid at top. Both KSA + UAE configs get new `quarterly-financials` tab. Backend: `build_thesis_metrics_from_pack()` extracts 21 flat metric keys for `ThesisTracker.check_drift`; `_load_thesis_summary()` returns read-only payload for frontend; `parse_tamara_data()` attaches `thesis_summary` to output. New `scripts/seed_tamara_thesis.py` idempotently seeds 8 pillars (statutory profitability > $0, CM% ≥ 3.5%, EBTDA > 0, GMV YTD vs budget ≥ -5%, Revenue YTD vs budget ≥ -10%, ECL coverage ≥ 2.5%, LTV/CAC ≥ 8x, BNPL+ mix ≥ 15%). `scripts/ingest_tamara_investor_pack.py` gains `--no-update-thesis` flag (default OFF = drift check fires); new `_run_thesis_drift_check()` prints alerts to stderr, failures don't fail ingest. Real-data smoke: all 8 pillars transitioned `holding → strengthening` (>20% headroom on every threshold), conviction 66 → 68.
  - **Classify CLI hardening** (commit `56dce47`, closes 2026-04-24 reclassify lesson): `dataroom_ctl classify` now passes `sheet_names` for xlsx files (new sheet rules fire during re-classify); `--dry-run` flag previews changes; help text recommends `--use-llm` to avoid regressing LLM-classified docs (~13 Tamara docs). Recommended flow: `classify --company X --use-llm --dry-run` → review → drop `--dry-run`.
  - **Tests:** 13 new thesis tests (`tests/test_tamara_thesis.py`) + 2 classify CLI tests (appended to `tests/test_dataroom_pipeline.py`) + 16 pack/classifier tests already landed in A-D. Full suite: **812 passing, 0 warnings** (was 797 pre-session; +15 new). Zero regressions.
  - **Fourth data ingestion pattern recognized:** alongside (1) raw tape (Klaim/SILQ), (2) pre-computed ODS summary (Ejari), (3) one-off data room ETL (Tamara original, Aajil), we now have (4) **recurring structured quarterly pack** — one parser handles all historical + future Tamara packs with identical template. Better pattern than the original Tamara one-off because re-runnable every quarter without touching code.
  - **Commits on `claude/tamara-investor-pack-ingest`:** `9d14144` (A-D), `e333595` (E+F), `e75bfb5` (E+F tests), `56dce47` (classify CLI), `3a88fce` (todo.md review). Plus ambient `d688519` (session-36 docs, not mine). Branch pollution incident documented in tasks/lessons.md 2026-04-24 entry — shared-checkout concurrent-agent work requires aggressive commit cadence.
- ✅ **Session 37 — Post-EoD continuation: prod ops + NotebookLM full purge + EoD step 11 fix (2026-04-25):**
  - **Production deployment + Tamara investor pack go-live.** First `./deploy.sh` pulled `670f57d..eca4dce`, surfaced that the alignment check was structurally insufficient (alignment OK but new files on disk were silently skipped). `sync-data.ps1` then pushed 139 raw files. 4 manual `docker compose exec` commands on prod (dataroom_ctl ingest + ingest_tamara_investor_pack.py + seed_tamara_thesis.py + ingest --force) registered the new files, generated the structured JSON, seeded the 8-pillar thesis, fired the drift check (all 8 strengthening, conviction 66→68/100). Bonus: dedupe pass auto-cleaned 142 stale sha-duplicate registry entries that had accumulated on prod (registry 271 → 130 → 129 after a surgical removal of one stale `notebooklm_state.json` registry entry whose source file was no longer on disk).
  - **EoD step 11 dataroom-sync check structural fix** (commit `ef315c8`). The check `git diff HEAD~10 | grep registry.json` had been broken since session 26.1 when `registry.json` was added to `.gitignore` — git diff never sees gitignored files, so the check returned empty for ~10 sessions, falsely reporting "no sync needed" every time. Replaced with filesystem-mtime check (`find data/*/dataroom/ingest_log.jsonl -mtime -1 -type f` plus a follow-up scan for source files newer than ingest_log). Also added explicit warning in EoD step 11 that **`deploy.sh` alone is NOT sufficient** after sync — alignment check only catches registry corruption, not new files. Post-sync command sequence (manual `dataroom_ctl ingest` + Tamara-specific pack/thesis commands) documented inline in the EoD step.
  - **NotebookLM full purge from active code** (commit `8b63d48`). Session 22b "NotebookLM Removal" (2026-04-16) had claimed "zero notebooklm references in codebase" but a grep this session surfaced 4 surviving references in ACTIVE code: (1) `docker/nginx.conf` `location /notebooklm` proxy_pass block — would 404 since backend has no handler but route survived; (2) `core/research/dual_engine.py` docstring still mentioned "Previously orchestrated Claude RAG + NotebookLM"; (3) my own session-37 defensive `notebooklm_state.json` exclusion in `_EXCLUDE_FILENAMES` (reverted per analyst direction); (4) same defensive entry in sync-data.ps1 ServerOwnedFiles (also reverted). After `8b63d48` the active codebase has zero NotebookLM references. Historical references in CLAUDE.md (Session 17 Klaim cleanup) and tasks/todo.md (Session 22b completion) preserved as audit trail.
  - **deploy.sh source-newer-than-registry detection — spawned as a separate task.** The deeper architectural fix (deploy.sh natively detecting new files instead of relying on the EoD step 11 reminder + manual prod commands) is filed as a CCD spawn task chip with a fully self-contained prompt. Approach: new `dataroom_ctl needs-ingest` subcommand that imports `_EXCLUDE_FILENAMES` from the engine + 8 regression tests + thin bash loop in deploy.sh. Branch when clicked: `claude/deploy-sh-needs-ingest-detection`. Not done this session — proper implementation needs focused time + bash testing on prod-shaped fixtures.
  - **Open at session close:** prod runs `eca4dce` not `8b63d48` (haven't redeployed since the post-EoD commits — functional impact zero, cosmetic only on /notebooklm route). Conviction left at 68/100 per analyst direction (system designed for slow conviction-earn over time; will tick up by 2 per quarterly drift check).
  - **Tests:** 812 passing (baseline maintained), 0 warnings, 0 regressions. **1 new lesson** in tasks/lessons.md: "EoD checks built on `git diff` against gitignored files silently break forever" — write filesystem-mtime checks instead of git-diff for any gitignored file.
...
- ✅ **Session 35 — §17 meta-audit walker + 45 rate-field disclosure fixes + propagation doctrine (2026-04-24):**
  - **User feedback triggered session:** after session 34 declared §17 complete, user looked at SILQ Credit Quality dashboard and flagged that PAR cards showed only active-denominator view while Aajil had a dual-view treatment. This had slipped past session 34's per-company manual audit because per-company audits systematically miss cross-company propagation.
  - **Three commits landed** on main (`1377ae3..666bb28`):
    1. **`1377ae3`** — committed the Apr 15 Klaim tape (had been on disk since Apr 20 but never `git add`'d; all prior Klaim tapes ARE tracked in git) + fixed EoD skill's incorrect "tapes are gitignored" claim. Closes the Tape-vs-Portfolio Analytics data-freshness gap on prod.
    2. **`1793ec4`** — SILQ PAR dual-view (the user-flagged gap). `compute_silq_summary` gains par30_amount/60/90 (absolute SAR), lifetime_par30/60/90 (total_disbursed denominator), total_active_outstanding, par_lifetime_population='total_originated'. Frontend Credit Quality subtitle now uses the actual at-risk amount (not the broken `par% × total_outstanding` math) + shows "· Lifetime: X.XX%" as dual-view context. **SILQ keeps active as PRIMARY** (lending product where live book IS the IC metric) — different primary-view pattern from Klaim (factoring, promotes lifetime to primary).
    3. **`666bb28`** — systematic §17 meta-audit walker + 45 rate-field fixes + Framework §17 additions.
  - **The walker** (`tests/test_population_discipline_meta_audit.py`, 700+ lines): introspects every compute_* function on every asset class, identifies rate-like fields via 50+ canonical names + suffixes, checks 5 disclosure modes (per-field, dict-level, par_confidence prefix, hhi_confidence prefix, sub-dict like yield_confidence). First run surfaced 45 gaps.
  - **45 rate-field disclosure fixes**: Klaim 17 (compute_summary + compute_returns_analysis + compute_denial_funnel + compute_par BOTH direct+derived branches + compute_cohort_loss_waterfall.totals + compute_klaim_stale_exposure + compute_stress_test + compute_cdr_ccr.portfolio), SILQ 17 (compute_silq_summary + compute_silq_delinquency with NEW lifetime dual + compute_silq_yield + compute_silq_cohort_loss_waterfall.totals + compute_silq_cdr_ccr.portfolio), Aajil 11 (compute_aajil_summary with NEW 3-pop collection_rate dual + compute_aajil_concentration + compute_aajil_loss_waterfall).
  - **8 new meta-tests in 3 classes**:
    - `TestMetaAuditRateFieldDisclosure` (1): walker enforcement; future rate-field additions without §17 tags fail CI with function+field name in error.
    - `TestMetaAuditDualPropagation` (6): PAR lifetime dual on every live-tape company, HHI clean dual on every company, collection_rate_realised on SILQ+Aajil, primitive-set completeness (separate+classify+operational_wal+methodology_log) on every live-tape company, summary-fn-must-mirror-compute-fn rule.
    - `TestMetaAuditTaxonomyFreshness` (1): emitted population codes must be pre-approved; forces explicit vote-in of new vocabulary.
  - **Framework §17 doctrine additions** (`core/ANALYSIS_FRAMEWORK.md`):
    - NEW "Principle propagation discipline" subsection: 4 rules codifying platform-wide dual-view rollout, summary-mirror-compute rule, primitive-completeness for live-tape asset classes, read-only exemption declaration.
    - NEW "Dual-view pattern taxonomy" subsection: 3 patterns with when-to-use guidance — Pattern 1 single-primary+context (lending: SILQ PAR, Aajil Operational WAL), Pattern 2 parallel-equal (factoring: Klaim PAR), Pattern 3 N-way comparison (yield-style: Aajil yield, Aajil collections). Prevents future pattern mismatch at onboarding time.
  - **Audit document**: `reports/session_35_meta_rethink_2026-04-24.md` documents the rethink process, findings by phase, meta-tests added, Framework §17 additions, and onboarding implications.
  - **3 new lessons** in `tasks/lessons.md`:
    1. Build walkers before eyeballing audits (walker found 45 gaps I couldn't have enumerated by hand).
    2. Summary fns must mirror compute fns on dual-view surfaces (the Aajil summary gap).
    3. Three dual-view patterns, not one (picking the wrong pattern creates analyst confusion).
  - **Tests: 780 passing** (was 772 + 8 new), 82 skipped, 0 warnings (baseline maintained), 0 regressions at every commit.
  - **Platform state: §17 walker passes cleanly across all 3 live-tape asset classes.** New compute functions + new asset classes onboarded in future sessions will automatically be walked by the meta-audit — any rate field without §17 tags fails CI before commit. No more "user finds gap in prod later".
...
- ✅ **Five data ingestion patterns now supported:**
  ...
  - **Data Room Ingestion** (Tamara, baseline): ~100 multi-format files → one-off ETL script → JSON snapshot → parser → dashboard
  - **Investor Deck Extraction** (Aajil): Single PDF → manual data extraction script → JSON snapshot → parser → dashboard
  - **Recurring Structured Pack** (Tamara, ongoing): Quarterly/monthly investor pack Excel with stable 10-sheet template → `scripts/ingest_tamara_investor_pack.py` produces timestamped JSON at `data/{company}/investor_packs/` + auto-fires thesis drift check. Re-runnable each period; template validation + 3-tier pack-date resolution + deterministic output. Session 37.
...
**Session 36 — 8-gap follow-up sweep after §17 audit waves ✅ COMPLETE (2026-04-24):**
- [x] **Gap 1 — analytics_bridge §17 transmission to non-covenant sections** (`f7ef580`). `_get_disclosure` + `_attach_disclosure` helpers; 5 section builders (portfolio_analytics / credit_quality / concentration / stress / covenants) now flow confidence+population to memo prompts. Family-level inheritance for par_*, hhi_*, collection_rate_*.
- [x] **Gap 2 — per-row walker extension** (`f7ef580`). `TestMetaAuditPerRowRateFieldDisclosure` recurses into dict-nested list values. Surfaced + fixed 19 additional gaps in 7 compute functions (Klaim facility_pd/cohort_loss_waterfall; SILQ concentration/tenure/borrowing_base/cohort_loss_waterfall; Aajil underwriting/yield). 5 disclosure-fix patterns documented: dict-level inheritance, list-level `<list>_population`, row-level, per-field, documented exemption.
- [x] **Gap 3 — ConfidenceBadge on chart panels** (`4a43f02`). Wired via ChartPanel `action` slot into 8 panels: CohortTable, ConcentrationChart (Group + Provider), CohortLossWaterfallChart, ReturnsAnalysisChart (IRR by Vintage + Monthly Returns), UnderwritingDriftChart, silq/DelinquencyChart (DPD + Monthly Trend). §17 now visible on charts, not just overview KPIs.
- [x] **Gap 4 — visual/browser verification** — compile-verified (vite build green, 1110 modules) + API data-flow verified (SILQ lifetime_par30=1.45% vs active 7.81%, Klaim 2.37% vs 36.59%, Aajil par_1_inst_lifetime=0.07 vs active 0.33). Full browser click-through handed off to user per session-36 kickoff.
- [x] **Gap 5 — FRAMEWORK_INDEX.md §17 cross-refs** (`7d68c9c`). §17 row in section-index table updated with "principle propagation discipline" (session 35) + "dual-view pattern taxonomy" (session 36 revision) subsection references. New Core Principle #8 "Credit Quality PAR: lifetime-primary universal", subsequent principles renumbered to keep 13-item list monotonic.
- [x] **Gap 6 — Ejari §17 exemption** (`7d68c9c`). Explicit exemption docstring added to `parse_ejari_workbook`. Ejari is pre-computed upstream; parser reshapes aggregated data. Meta-audit walker correctly omits Ejari functions (no `_ejari_functions()` in rotation, intentionally).
- [x] **Gap 7 — `tasks/next-session-kickoff.md` refresh** (`074f430`). Stale session-27 external.web_search prompt archived as `<details>`. Current body has session-36 carry-over with visual verification handoff checklist + pattern guidance for future audits.
- [x] **Gap 8 — Credit Quality PAR lifetime-primary across all companies** (`4a43f02`). SILQ: `TapeAnalytics.jsx` flipped (headline = `lifetime_par30 ?? par30`, subtitle = "at risk · Active: X.XX%", sublabel = "vs Total Disbursed (lifetime)"). Klaim: enhanced subtitle to match SILQ pattern. Aajil: Delinquency tab PAR 1+/2+/3+ Inst cards flipped to `par_N_inst_lifetime` primary. Ejari (single-PAR upstream §17-exempt) + Tamara (DPD distribution, no dual cards) correctly skipped.
- [x] **Gap 8a — §17 dual-view pattern taxonomy revision** (`7d68c9c`). Session 35 codified Pattern 1 (active-primary for lending). Session 36 override: lifetime-primary universal for PAR family across all asset classes. Rationale: cross-company IC consistency; lifetime preserves historical loss signal that active resets away; active remains available for covenant compliance unchanged.
- [x] **743 pytest pass** (baseline maintained), 0 warnings, 0 regressions.
- [x] **3 new lessons in `tasks/lessons.md`** — walkers must cover all shapes; concurrent Claude Code sessions share working tree state; doctrine + implementation in same landing.

**Session 35 — Systematic §17 meta-audit walker + 45 rate-field fixes ✅ COMPLETE (2026-04-24):**
- [x] **Apr 15 Klaim tape tracked** (`1377ae3`) — pre-session gap closed. Tape had been on disk 4 days without being `git add`'d; EoD skill's policy claim corrected in same commit.
- [x] **SILQ PAR dual-view** (`1793ec4`) — user-flagged session 34 miss. compute_silq_summary gains par*_amount + lifetime_par* + total_active_outstanding + par_lifetime_population. Frontend Credit Quality subtitle arithmetic fixed + lifetime context appended.
- [x] **§17 meta-audit walker** (`666bb28`) — `tests/test_population_discipline_meta_audit.py` introspects every compute_* function, identifies rate-like fields via 50+ canonical names + 5 disclosure modes. First run found 45 gaps across Klaim/SILQ/Aajil.
- [x] **45 rate-field disclosure fixes** (`666bb28`) — every top-level compute function now declares §17 disclosure for its rate fields (dict-level confidence+population, per-field variants for lifetime/clean duals, or explicit exemption for descriptive stats).
- [x] **SILQ compute_silq_delinquency lifetime PAR dual** (`666bb28`) — closes the Delinquency tab gap that the summary-only fix hadn't covered.
- [x] **Aajil compute_aajil_summary 3-population collection_rate dual** (`666bb28`) — mirrors compute_aajil_collections P1-4 pattern. Closes the summary-fn-must-mirror-compute-fn gap.
- [x] **TestMetaAuditDualPropagation** (6 tests in `666bb28`) — asserts dual views exist on every applicable asset class (PAR lifetime, HHI clean, collection_rate_realised, operational_wal triad, methodology_log, summary-mirror rule). Locks in cross-company propagation.
- [x] **TestMetaAuditTaxonomyFreshness** — new population codes must be voted into _ALLOWED_POPULATIONS_PREFIX before compute functions can emit them.
- [x] **Framework §17 principle propagation discipline** codified in `core/ANALYSIS_FRAMEWORK.md` — 4 rules: dual-view platform-wide rollout, summary-mirror-compute, primitive-completeness for live-tape companies, read-only exemption declaration.
- [x] **Framework §17 dual-view pattern taxonomy** codified — 3 patterns (single-primary+context lending-style, parallel-equal factoring-style, N-way yield-style) with when-to-use guidance.
- [x] **Audit report** `reports/session_35_meta_rethink_2026-04-24.md`.
- [x] **3 lessons in `tasks/lessons.md`** — build walkers before eyeballing audits, summary-mirror-compute, three dual-view patterns not one.
- [x] **780 tests passing** (was 772 + 8 new), 82 skipped, 0 warnings, 0 regressions.
