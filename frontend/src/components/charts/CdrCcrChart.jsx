...
import { getCdrCcr } from '../../services/api'
import {
...
export default function CdrCcrChart({ company, product, snapshot, currency, asOfDate }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getCdrCcr(company, product, snapshot, currency, asOfDate)
  ...
  // Chart data: one bar per vintage, CDR red, CCR teal
  ...
  return (
    <ChartPanel title="CDR / CCR — Conditional Rates by Vintage" loading={loading} error={error}>
      ...
      <div style={{
        ...
        CDR and CCR annualize each vintage's cumulative rates by its age (months outstanding), making young
        and mature cohorts directly comparable. A 6-month vintage with 5% cumulative defaults has CDR = 10%
        — higher than a 12-month vintage at 8% (CDR = 8%). Net spread = CCR − CDR; positive spread means
        collections outpace defaults on a normalized basis.
      </div>
      ...
      <ResponsiveContainer width="100%" height={340}>
        <ComposedChart data={chartData} margin={{ top: 8, right: 20, left: 0, bottom: 40 }}>
          ...
          />
          <Legend {...legendProps} />
          {/* Shaded area between CDR and CCR (net spread) */}
          <Area
            type="monotone"
            dataKey="CCR"
            fill={COLORS.teal}
