...
def build_executive_summary_prompt(
    ...
    return (
        ...
        "\n"
        "POPULATION & CONFIDENCE DISCLOSURE (Framework §17):\n"
        "- Every metric output dict now carries a `population` field (one of: total_originated, "
        "active_outstanding, active_pv, completed_only, clean_book, loss_subset, zombie_subset, "
        "snapshot_date_state, specific_filter(...), manual(...)) and a `confidence` field "
        "(A observed / B inferred / C derived).\n"
        "- For EACH narrative section, end the prose with a one-line 'Methodology:' footer that states "
        "the dominant population + confidence grade for the metrics cited. Example: "
        "'Methodology: PAR reported on active_outstanding (Confidence B — age_pending proxy). "
        "Lifetime denominator shown alongside.'\n"
        "- When TWO populations are cited for the same metric (e.g. Active + Lifetime PAR, "
        "blended + clean cohort rates, Operational + Realized WAL), name BOTH populations and "
        "confidence grades in the section's Methodology footer.\n"
        "- If a covenant is measured with Confidence C (e.g. Klaim Collection Ratio cumulative "
        "approximation), surface that in the Bottom Line — an IC reading a C-graded covenant needs "
        "to know the number is a single-snapshot substitute, not the period-based true rate.\n"
        "- Do NOT pollute the main narrative prose with confidence brackets on every number — "
        "reserve brackets for cases where the grade materially affects interpretation "
        "(B/C grades on headline covenants).\n"
    )
