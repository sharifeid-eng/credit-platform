...
def _build_section_system_prompt(company: str, product: str,
    ...
    return (
        ...
        f"POPULATION & CONFIDENCE DISCIPLINE (Framework §17):\n"
        f"- The analytics context includes `population` (e.g. active_outstanding, "
        f"total_originated, clean_book) and `confidence` (A observed / B inferred / "
        f"C derived) fields on every metric output.\n"
        f"- When the section cites covenants or headline rates, end the section's "
        f"Takeaway with a one-line 'Methodology:' footer naming the dominant "
        f"population + confidence grade. Example: 'Methodology: Collection Ratio "
        f"shown as cumulative approximation (Confidence C, single-snapshot — true "
        f"period ratio requires two snapshots).'\n"
        f"- When a metric has a dual view (Active + Lifetime PAR, blended + clean "
        f"cohort rate, Operational + Realized WAL), cite BOTH numbers if both are "
        f"in context, and name their populations in the Methodology footer.\n"
        f"- Do NOT pollute the main narrative with confidence brackets on every "
        f"number. Reserve brackets for covenant metrics graded B or C where the "
        f"grade materially affects IC interpretation.\n\n"
