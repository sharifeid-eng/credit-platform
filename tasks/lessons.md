...
**Fix:** Added Step 1b to `/eod` — explicit check for untracked data artifacts: `registry.json`, `notebooklm_state.json`, `mind/*.jsonl`, `relations.json`, `*_extracted.json`, `reports/memos/`. These are runtime-generated but represent expensive work product (AI extraction, NLM sync, data room ingestion).
**Rule:** Any file generated by an expensive operation (AI calls, NLM sync, data room ingestion) that can't be cheaply regenerated MUST be committed. The EOD step 1b glob pattern catches these.
